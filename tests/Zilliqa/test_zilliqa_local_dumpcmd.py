...
def run_start(numdsnodes):
	...
	for x in range(0, count):
		...
		if (x < numdsnodes):
			...
			os_system('run_start', 'cd ' + LOCAL_RUN_FOLDER + testfolders_list[x] + '; echo \"' + keypair[0] + ' ' + keypair[1] + '\" > mykey.txt' + '; ulimit -n 65535; ulimit -Sc unlimited; ulimit -Hc unlimited; $(pwd)/zilliqa ' + keypair[1] + ' ' + keypair[0] + ' ' + '127.0.0.1' +' ' + str(NODE_LISTEN_PORT + x) + ' 1 0 1 > ./error_log_zilliqa 2>&1 &')
		else:
			os_system('run_start', 'cd ' + LOCAL_RUN_FOLDER + testfolders_list[x] + '; echo \"' + keypair[0] + ' ' + keypair[1] + '\" > mykey.txt' + '; ulimit -n 65535; ulimit -Sc unlimited; ulimit -Hc unlimited; $(pwd)/zilliqa ' + keypair[1] + ' ' + keypair[0] + ' ' + '127.0.0.1' +' ' + str(NODE_LISTEN_PORT + x) + ' 0 0 1 > ./error_log_zilliqa 2>&1 &')
...
def run_connect(numnodes):
	...
	for x in range(0, numnodes):
		...
		if has_peers_to_connect:
			os_system('run_connect', connect_cmd + ' &')
			time.sleep(1)

	print 'Total num of edges connected: ' + str(len(edges))

def run_stop():
	os_system('run_stop', 'killall zilliqa')
	os_system('run_stop', 'killall sendtxn')
	if os.path.exists(LOCAL_RUN_FOLDER):
		testfolders_list = get_immediate_subdirectories(LOCAL_RUN_FOLDER)
		count = len(testfolders_list)
		for x in range(0, count):
			os_system('run_stop', 'fuser -k ' + str(NODE_LISTEN_PORT + x) + '/tcp')
...
def run_sendcmd(nodenum, msg):
	os_system('run_sendcmd', 'tests/Zilliqa/sendcmd ' + str(NODE_LISTEN_PORT + nodenum - 1) + ' cmd ' + msg)
	
def run_sendcmdrandom(nodenum, msg_size):
	# msg = "000400" + 'A' * msg_size * 2
	os_system('run_sendcmdrandom', 'tests/Zilliqa/sendcmd ' + str(NODE_LISTEN_PORT) + ' broadcast ' + msg_size)
...
def run_startpow1(nodenum, dscount, blocknum, diff, rand1, rand2):
	...
	# Send to node
	os_system('run_startpow1', startpow1_cmd)

def run_sendtxn(portnum):
	os_system('run_sendtxn', 'tests/Zilliqa/sendtxn ' + str(portnum) + ' &')

def run_delete():
	if (os.path.exists(LOCAL_RUN_FOLDER)):
		shutil.rmtree(LOCAL_RUN_FOLDER)

def os_system(caller, command):
	print caller + ': <' + command + '>'
	os.system(command)
