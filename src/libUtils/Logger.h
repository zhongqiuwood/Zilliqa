...
#define LOG_MESSAGE(msg)                                                           \
    {                                                                              \
        ...
        oss << msg;                                                                \
        Logger::GetLogger(NULL, true).LogMessage(oss.str().c_str(), __FUNCTION__); \
    }
#define LOG_MESSAGE2(blockNum, msg)                                                          \
    {                                                                                        \
        std::ostringstream oss;                                                              \
        oss << msg;                                                                          \
        Logger::GetLogger(NULL, true).LogMessage(oss.str().c_str(), __FUNCTION__, blockNum); \
    }
#define LOG_PAYLOAD(msg, payload, max_bytes_to_display)                                                                     \
    {                                                                                                                       \
        std::ostringstream oss;                                                                                             \
        oss << msg;                                                                                                         \
        Logger::GetLogger(NULL, true).LogMessageAndPayload(oss.str().c_str(), payload, max_bytes_to_display, __FUNCTION__); \
    }
#define LOG_STATE(msg)                                                                \
    {                                                                                 \
        std::ostringstream oss;                                                       \
        oss << msg;                                                                   \
        Logger::GetStateLogger(NULL, true).LogState(oss.str().c_str(), __FUNCTION__); \
    }
