...
void DirectoryService::StoreDSBlockToStorage()
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Storing DS Block Number: " << m_pendingDSBlock->GetHeader().GetBlockNum() << " with Nonce: " << m_pendingDSBlock->GetHeader().GetNonce() << ", Difficulty: " << m_pendingDSBlock->GetHeader().GetDifficulty() << ", Timestamp: " << m_pendingDSBlock->GetHeader().GetTimestamp());
                                                     ...
                                                     unsigned int& my_pow1nodes_cluster_hi) const
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "New DSBlock created with chosen nonce   = 0x" << hex << m_mediator.m_dsBlockChain.GetLastBlock().GetHeader().GetNonce());
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "New DSBlock hash is                     = 0x" << DataConversion::charArrToHexStr(m_mediator.m_dsBlockRand));
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "New DS leader (PoW1 winner) IP          = " << winnerpeer.GetPrintableIPAddress() << ":" << winnerpeer.m_listenPortHost);
...
#ifdef STAT_TEST
        LOG_STATE("[IDENT][" << setw(15) << left << m_mediator.m_selfPeer.GetPrintableIPAddress() << "][" << setw(6) << left << m_consensusMyID << "] DSBK");
#endif // STAT_TEST
    }
...
void DirectoryService::ScheduleShardingConsensus(const unsigned int wait_window)
{
    ...
    auto func = [this, wait_window]() -> void {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Waiting " << wait_window << " seconds, accepting PoW2 submissions...");
                                                       ...
                                                       unsigned int offset)
{
    ...
    if (m_mode == PRIMARY_DS)
    {
        LOG_STATE("[DSCON][" << setw(15) << left << m_mediator.m_selfPeer.GetPrintableIPAddress() << "][" << m_mediator.m_txBlockChain.GetBlockCount() << "] DONE");
    }
#endif // STAT_TEST
    ...
    // To-do: Should store the bit map as
    {
        ...
        if (m_pendingDSBlock->GetHeader().GetBlockNum() == m_mediator.m_dsBlockChain.GetBlockCount() + 1)
        {
            LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
