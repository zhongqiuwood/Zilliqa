...
void Lookup::SendMessageToLookupNodes(const std::vector<unsigned char>& message) const
{
    ...
    for (auto node : m_lookupNodes)
    {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "Sending msg to lookup node " << node.GetPrintableIPAddress() << ":" << node.m_listenPortHost);
...
void Lookup::SendMessageToSeedNodes(const std::vector<unsigned char>& message) const
{
    ...
    {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "Sending msg to seed node " << node.GetPrintableIPAddress() << ":" << node.m_listenPortHost);
        P2PComm::GetInstance().SendMessage(node, message);
    }
}
                                       ...
                                       const Peer& from)
{
    ...
    if (IsMessageSizeInappropriate(message.size(), offset, UINT256_SIZE + UINT256_SIZE + sizeof(uint32_t)))
    {
        return false;
    }

    // 32-byte lower-limit block number
    boost::multiprecision::uint256_t lowBlockNum = Serializable::GetNumber<uint256_t>(message, offset, UINT256_SIZE);
    offset += UINT256_SIZE;

    // 32-byte upper-limit block number
    boost::multiprecision::uint256_t highBlockNum = Serializable::GetNumber<uint256_t>(message, offset, UINT256_SIZE);
    offset += UINT256_SIZE;
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "ProcessGetDSBlockFromSeed requested by " << from << " for blocks " << lowBlockNum.convert_to<string>() << " to " << highBlockNum.convert_to<string>());
    ...
    for (blockNum = lowBlockNum; blockNum <= highBlockNum; blockNum++)
    {
        ...
        {
            LOG_MESSAGE("Block Number " + blockNum.convert_to<string>() + " absent. Didn't include it in response message. Reason: " << e);
            break;
        }
    }
                                       ...
                                       const Peer& from)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "ProcessGetTxBlockFromSeed requested by " << from << " for blocks " << lowBlockNum.convert_to<string>() << " to " << highBlockNum.convert_to<string>());
                                       ...
                                       const Peer& from)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "ProcessSetDSBlockFromSeed sent by " << from << " for blocks " << lowBlockNum.convert_to<string>() << " to " << highBlockNum.convert_to<string>());
         ...
         blockNum++)
    {
        ...
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "dsblock.GetHeader().GetDifficulty(): " << (int)dsBlock.GetHeader().GetDifficulty());
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "dsblock.GetHeader().GetNonce(): " << dsBlock.GetHeader().GetNonce());
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "dsblock.GetHeader().GetBlockNum(): " << dsBlock.GetHeader().GetBlockNum());
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "dsblock.GetHeader().GetMinerPubKey().hex(): " << dsBlock.GetHeader().GetMinerPubKey());
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "dsblock.GetHeader().GetLeaderPubKey().hex(): " << dsBlock.GetHeader().GetLeaderPubKey());
                                       ...
                                       const Peer& from)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "ProcessSetTxBlockFromSeed sent by " << from << " for blocks " << lowBlockNum.convert_to<string>() << " to " << highBlockNum.convert_to<string>());
