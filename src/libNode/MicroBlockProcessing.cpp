...
#ifdef STAT_TEST
    LOG_STATE("[MICRO][" << std::setw(15) << std::left << m_mediator.m_selfPeer.GetPrintableIPAddress() << "][" << m_mediator.m_currentEpochNum << "] SENT");
#endif // STAT_TEST
...
bool Node::ComposeMicroBlock()
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Micro block proposed with " << m_microblock->GetHeader().GetNumTxs() << " transactions for epoch " << m_mediator.m_currentEpochNum);
                             ...
                             const Peer& from)
{
    ...
    for (uint32_t i = 0; i < numOfAbsentHashes; i++)
    {
        ...
        {
            LOG_MESSAGE("Leader unable to find txn proposed in microblock " << missingTransactions[i]);
            throw exception();
        }
...
bool Node::RunConsensusOnMicroBlockWhenShardLeader()
{
    ...
    auto nodeMissingTxnsFunc = [this](const vector<unsigned char>& errorMsg, unsigned int offset,
                                      const Peer& from) mutable -> bool { return OnNodeMissingTxns(errorMsg, offset, from); };

    auto commitFailureFunc = [this](const map<unsigned int, vector<unsigned char>>& m) mutable -> bool { return OnCommitFailure(m); };
...
bool Node::RunConsensusOnMicroBlockWhenShardBackup()
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "I am a backup node. Waiting for microblock announcement for epoch " << m_mediator.m_currentEpochNum);
    //m_consensusID = 0;
    m_consensusBlockHash.resize(BLOCK_HASH_SIZE);
    fill(m_consensusBlockHash.begin(), m_consensusBlockHash.end(), 0x77);
    auto func = [this](const vector<unsigned char>& message,
                       vector<unsigned char>& errorMsg) mutable -> bool { return MicroBlockValidator(message, errorMsg); };
...
bool Node::CheckBlockTypeIsMicro()
{
    ...
    {
        LOG_MESSAGE("Error: Type check failed. Expected: " << (unsigned int)TXBLOCKTYPE::MICRO << " Actual: " << (unsigned int)m_microblock->GetHeader().GetType());
        return false;
    }
...
bool Node::CheckMicroBlockVersion()
{
    ...
    {
        LOG_MESSAGE("Error: Version check failed. Expected: " << (unsigned int)BLOCKVERSION::VERSION1 << " Actual: " << (unsigned int)m_microblock->GetHeader().GetVersion());
        return false;
    }
...
bool Node::CheckMicroBlockTimestamp()
{
    ...
    if (m_mediator.m_txBlockChain.GetBlockCount() > 0)
    {
        ...
        {
            LOG_MESSAGE("Error: Timestamp check failed. Last Tx Block: " << lastTxBlockTimestamp << " Microblock: " << thisMicroblockTimestamp);
            return false;
        }
    }
...
bool Node::CheckMicroBlockHashes(vector<unsigned char>& errorMsg)
{
    ...
    {
        LOG_MESSAGE("Error: Tx hashes check failed. Tx hashes size: " << txhashessize << " Num txs: " << numtxs);
        return false;
    }
...
bool Node::CheckMicroBlockTxnRootHash()
{
    ...
    LOG_MESSAGE("Microblock root computation done " << DataConversion::charArrToHexStr(expectedTxRootHash.asArray()));
    LOG_MESSAGE("Expected root: " << DataConversion::charArrToHexStr(m_microblock->GetHeader().GetTxRootHash().asArray()));
                               ...
                               vector<unsigned char>& errorMsg)
{
    ...
    do
    {
        if (!CheckBlockTypeIsMicro() || !CheckMicroBlockVersion() || !CheckMicroBlockTimestamp() || !CheckMicroBlockHashes(errorMsg) || !CheckMicroBlockTxnRootHash())
        {
            break;
        }
