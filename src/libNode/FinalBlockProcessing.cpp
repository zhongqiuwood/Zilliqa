                                               ...
                                               unsigned int& cur_offset, uint8_t& shard_id)
{
    ...
    if (consensusID != m_consensusID)
    {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "Consensus ID is not correct. Expected ID: " << consensusID << " My Consensus ID: " << m_consensusID);
        return false;
    }
...
void Node::StoreFinalBlock(const TxBlock& txBlock)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "DEBUG last block has a size of " << m_mediator.m_txBlockChain.GetLastBlock().GetSerializedSize())
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "DEBUG cur block has a size of " << txBlock.GetSerializedSize())
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "Storing Tx Block Number: " << txBlock.GetHeader().GetBlockNum() << " with Type: " << txBlock.GetHeader().GetType() << ", Version: " << txBlock.GetHeader().GetVersion() << ", Timestamp: " << txBlock.GetHeader().GetTimestamp() << ", NumTxs: " << txBlock.GetHeader().GetNumTxs());
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Final block " << m_mediator.m_txBlockChain.GetLastBlock().GetHeader().GetBlockNum() << " received with prevhash 0x" << DataConversion::charArrToHexStr(m_mediator.m_txBlockChain.GetLastBlock().GetHeader().GetPrevHash().asArray()));

#ifdef STAT_TEST
    LOG_STATE("[FINBK][" << std::setw(15) << std::left << m_mediator.m_selfPeer.GetPrintableIPAddress() << "][" << m_mediator.m_txBlockChain.GetLastBlock().GetHeader().GetBlockNum() << "] RECV");
#endif // STAT_TEST
}
                                                 ...
                                                 const boost::multiprecision::uint256_t& blocknum)
{
    ...
    if (m_unavailableMicroBlocks.find(blocknum) != m_unavailableMicroBlocks.end() && m_unavailableMicroBlocks[blocknum].size() > 0)
    {
                                   ...
                                   const boost::multiprecision::uint256_t& blocknum)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "Expected FinalBlock TxRoot hash : " << DataConversion::charArrToHexStr(microBlocksHash.asArray()));
                                      ...
                                      const TxnHash& tx_hash)
{
    ...
    if (txnIt != submittedTransactions.end())
    {
        ...
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "[TXN] [" << blockNum << "] Committed     = 0x" << DataConversion::charArrToHexStr(committedTransactions.back().GetTranID().asArray()));
                                     ...
                                     const TxnHash& tx_hash)
{
    ...
    if (txnIt != receivedTransactions.end())
    {
        ...
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "[TXN] [" << blockNum << "] Committed     = 0x" << DataConversion::charArrToHexStr(committedTransactions.back().GetTranID().asArray()));
        ...
        **/
        ...
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "ReceivedTransaction: Storing Transaction: " << DataConversion::charArrToHexStr(tx_hash.asArray()) << " with amount: " << committedTransactions.back().GetAmount() << ", to: " << committedTransactions.back().GetToAddr() << ", from: " << committedTransactions.back().GetFromAddr());
                                    ...
                                    uint8_t sharing_mode, vector<Transaction>& txns_to_send)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "Number of transactions to broadcast for block " << blocknum << " = " << txns_to_send.size());
                                                    ...
                                                    vector<Transaction>& txns_to_send) const
{
    ...
    if (txns_to_send.size() > 0)
    {
        ...
        for (unsigned int i = 0; i < txns_to_send.size(); i++)
        {
            ...
            LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "[TXN] [" << blocknum << "] Broadcasted   = 0x" << DataConversion::charArrToHexStr(txns_to_send.at(i).GetTranID().asArray()));
        }
                                                  ...
                                                  const uint256_t& blocknum)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "[shard " << m_myShardID << "] I am a forwarder for transactions in block " << blocknum);
...
bool Node::IsMyShardsMicroBlockTxRootHashInFinalBlock(const uint256_t& blocknum,
                                                      bool& isEveryMicroBlockAvailable)
{
    return m_microblock != nullptr && IsMicroBlockTxRootHashInFinalBlock(m_microblock->GetHeader().GetTxRootHash(), blocknum, isEveryMicroBlockAvailable);
}
...
void Node::LogReceivedFinalBlockDetails(const TxBlock& txblock)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "txblock.GetHeader().GetNumMicroBlockHashes(): " << txblock.GetHeader().GetNumMicroBlockHashes());
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                 "txblock.GetHeader().GetStateRootHash(): " << txblock.GetHeader().GetStateRootHash());
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
...
bool Node::CheckStateRoot(const TxBlock& finalBlock)
{
    ...
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "State root matched " << finalBlock.GetHeader().GetStateRootHash());
                             ...
                             const Peer& from)
{
    ...
    if (m_state == MICROBLOCK_CONSENSUS)
    {
        ...
        while (m_state != WAITING_FINALBLOCK)
        {
            ...
            if (time_pass % 10 == 1)
            {
                LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), string("Waiting ") + "for state change from MICROBLOCK_CONSENSUS to PROCESS_FINALBLOCK");
            }
                                         ...
                                         vector<Transaction>& txnsInForwardedMessage)
{
    ...
    cur_offset += TRAN_HASH_SIZE;
    LOG_MESSAGE("Received MicroBlock TxHash root : " << DataConversion::charArrToHexStr(microBlockTxHash.asArray()));
                                       ...
                                       const uint256_t& blocknum)
{
    ...
    for (const auto& tx : txnsInForwardedMessage)
    {
        ...
        {
            LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Proceessed " << txn_counter << " of txns.");
        }
    }
}
                                     ...
                                     const Peer& from)
{
    ...
    if (m_mediator.m_txBlockChain.GetLastBlock().GetHeader().GetBlockNum() < blocknum)
    {
        ...
        while (m_mediator.m_txBlockChain.GetLastBlock().GetHeader().GetBlockNum() < blocknum)
        {
            ...
            if (time_pass % 10 == 1)
            {
                LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Blocknum " + blocknum.convert_to<string>() + " waiting " + "for state change from WAITING_FINALBLOCK to TX_SUBMISSION");
            }
            this_thread::sleep_for(chrono::milliseconds(100));
        }
    }
