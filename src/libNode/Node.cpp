...
bool Node::CheckState(Action action)
{
    ...
    {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), string("Error: I am a DS node.") + string(" Why am I getting this message? Action: ") << ActionString(action));
        return false;
    }
...
{
    m_state = state;
    LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(), "Node State is now " << m_state << " at epoch " << m_mediator.m_currentEpochNum);
}
...
#ifdef STAT_TEST
    LOG_STATE("[TXNSE][" << std::setw(15) << std::left << m_mediator.m_selfPeer.GetPrintableIPAddress() << "][" << m_mediator.m_currentEpochNum << "][" << m_myShardID << "][" << txn_sent_count << "] CONT");
#endif // STAT_TEST
}
#endif // IS_LOOKUP_NODE
